...
probpool = function(env_pool = NULL, disp_pool = NULL, occurrences = NULL,
  ...
  if(is.null(interaction_matrix) && (!is.null(env_pool) || !is.null(disp_pool))){ # No interactions, multiply env * disp
    ...
    if(!is.null(env_pool) || !is.null(disp_pool)){ # Base probabilities from env/disp layer
      ...
      prob_pool.raw = calc(occurrences, function(x){ # Assign uniform distribution to raster
        x[!is.na(x)] = base.prob
        return(x)
      }) 
...
setMethod("plot", "Probpool", function(x, focal.species = NULL, focal.unit = NULL, ...){
  ...
  if(is.null(focal.species) && is.null(focal.unit)){
    ...
    richness.maps = stack(x@species_richness[!is.na(x@species_richness)])
    ...
    focal.pdf = apply(focal.probs, 2, ppoibin, kk = 1:x@species_total)
    focal.pdf = melt(focal.pdf)
    focal.pdf$type = "Probability density"
    focal.cdf = apply(focal.probs, 2, dpoibin, kk = 1:x@species_total)
    focal.cdf = melt(focal.cdf)
    focal.cdf$type = "Cumulative probability density"
    
    focal.all = rbind(focal.pdf, focal.cdf)
    xyplot(value ~ Var1 | factor(Var2) + factor(type), data = focal.all, ylab = "probability", xlab= "species", type = "l")
  } else {
